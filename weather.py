...
def main():
    ...
    while True:
        ...
        uv_index = float(input("Enter the value of uv index: "))
        rainfall = float(input("Enter the value og rainfall: "))
        wind_Speed = float(input("Enter the value of  wind speed: "))
    ...
    for date, data in records.items():
        ...
        print(f"{formatted_date}\t| {temperature:.1f}°C\t| {humidity:.1f}%\t| {uv_index:.1f}mW/m**2\t| {rainfall:.1f}mm\t| {wind_Speed:.1f}m/s")
